...
let mode = 'console'; // 'resend' | 'smtp' | 'console'
...
function init() {
  ...
  if (!host || process.env.MAIL_MODE === 'console') {
    ...
    mode = 'console';
    console.warn('\n[MAILER] ATENÇÃO: envio de email DESATIVADO (sem RESEND_API_KEY ou MAIL_HOST).');
    console.warn('[MAILER] Os códigos de verificação só aparecerão nos LOGS do servidor.');
    console.warn('[MAILER] Na Railway é obrigatório usar RESEND_API_KEY (SMTP é bloqueado).\n');
    return;
  }
  mode = 'smtp';
...
async function sendViaResend(email, code) {
  const { subject, text, html } = render(email, code);
  const from = process.env.RESEND_FROM || 'Nexus Control <onboarding@resend.dev>';
...
async function sendCode(email, code) {
  try {
    ...
    console.error(`[MAILER] Falha ao enviar email (${mode}) para`, email, '-', e.message);
    throw new Error('Falha ao enviar o email: ' + e.message);
  }
  return { mode };
}
